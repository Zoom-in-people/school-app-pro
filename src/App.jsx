...
import MemoPage from './pages/MemoPage'; // 🔥 신규 추가
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const applyTheme = (theme) => { if (theme === 'dark') root.classList.add('dark'); else if (theme === 'light') root.classList.remove('dark'); else { if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) root.classList.add('dark'); else root.classList.remove('dark'); } };
  ...
  if (!user) return <div className="flex h-screen items-center justify-center"><button onClick={login} className="bg-indigo-600 text-white p-4 rounded font-bold">Google 로그인</button></div>;
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 font-sans transition-colors duration-300 print:h-auto print:bg-white print:text-black">
      ...
      <div className={`no-print fixed md:relative inset-y-0 left-0 z-50 bg-white dark:bg-gray-800 border-r shadow-2xl md:shadow-none transform transition-all duration-300 shrink-0 ${store.isSidebarOpen ? 'translate-x-0 w-64' : '-translate-x-full md:translate-x-0 md:w-20'}`}>
        <Sidebar user={user} logout={logout} handbooks={handbooks} />
      </div>

      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        <header className="md:hidden bg-white p-4 flex items-center justify-between border-b sticky top-0 z-30">
        ...
        <div className="flex-1 p-4 md:p-6 lg:p-8 overflow-y-auto">
          <div className="max-w-7xl mx-auto h-full">
            {store.activeView === 'unified_settings' ? <UnifiedSettings store={store} handbook={store.currentHandbook} onUpdateHandbook={handleUpdateHandbook} onDeleteHandbook={handleDeleteHandbook} />
            : store.activeView === 'ai_record' ? <AiRecord />
            : store.activeView === 'memos' ? <MemoPage /> // 🔥 메모 페이지 연결
