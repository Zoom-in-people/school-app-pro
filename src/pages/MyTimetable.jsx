...
export default function MyTimetable({ timetableData = [], onAddTimetable, onUpdateTimetable, onDeleteTimetable }) {
  ...
  const [settings, setSettings] = useState({
    days: ['월', '화', '수', '목', '금'],
    totalPeriods: 6, classDuration: 45, breakTime: 10, startTime: '09:00', lunchStart: '12:30', lunchEnd: '13:20', lunchAfter: 4      
  });
  ...
  const handleTimetableUpload = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      const newTimetable = { type: 'manual', settings: newSettings, schedule: newSchedule, recentSubjects: timetable?.recentSubjects || [], recentRooms: timetable?.recentRooms || [] };
  ...
  const handleDownloadTimetableTemplate = () => {
     let aoa = [["교시", ...settings.days]];
     for(let i=1; i<=settings.totalPeriods; i++) {
       let row = [`${i}교시`];
       settings.days.forEach(day => {
         const cell = timetable?.schedule?.[`${i}-${day}`];
         row.push(cell ? `${cell.subject}${cell.room ? '\n' + cell.room : ''}` : "");
       });
       aoa.push(row);
     }
        ...
        <div className="flex-1 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 md:p-8 overflow-y-auto custom-scrollbar">
          <div className="max-w-3xl mx-auto space-y-6">
            ...
            <div className="flex justify-end gap-3 pt-2">
              {timetable && timetable.id && <button onClick={() => setIsSettingMode(false)} className="px-6 py-3 font-bold text-gray-500 hover:bg-gray-100 rounded-xl transition">취소</button>}
              <button onClick={handleSaveSettings} className="bg-indigo-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-indigo-700 transition flex items-center gap-2 shadow-lg shadow-indigo-200 dark:shadow-none">완료 및 표 만들기 <ChevronRight size={20}/></button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="h-full flex flex-col gap-4 relative">
      <div className="flex flex-wrap justify-between items-center bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 shrink-0">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={handleReset} className="bg-red-50 text-red-600 hover:bg-red-100 px-4 py-2 rounded-lg font-bold flex items-center gap-2 transition"><Trash2 size={18}/> 초기화</button>
        </div>
      </div>
          ...
          <table className="w-full text-center h-full table-fixed">
            <thead className="bg-indigo-50 dark:bg-gray-700/50">
              <tr>
                ...
                {settings.days.map(day => <th key={day} className="p-2 border-b border-r dark:border-gray-600 last:border-r-0 text-gray-700 dark:text-gray-200 font-extrabold text-sm">{day}</th>)}
              </tr>
            </thead>
            <tbody>
                  ...
                  <React.Fragment key={timeInfo.period}>
                    <tr className="hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors">
                      ...
                      {settings.days.map(day => {
                        ...
                        return (
                          <td key={day} onClick={() => handleCellClick(day, timeInfo.period)} className="p-1 border-b border-r dark:border-gray-600 last:border-r-0 cursor-pointer group hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-all relative">
                            {cellData ? (
                              ...
                              <div className="w-full h-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><div className="bg-indigo-100 text-indigo-500 dark:bg-indigo-900 dark:text-indigo-300 p-1.5 rounded-full"><Plus size={16}/></div></div>
                            )}
                          </td>
                        );
                      })}
                    ...
                    {isLunchNext && (
                      <tr className="bg-orange-50/50 dark:bg-orange-900/10">
                        <td colSpan={settings.days.length + 1} className="p-1 border-b dark:border-gray-600 text-center h-14 md:h-16">
                          <div className="flex items-center justify-center gap-2 text-orange-600 dark:text-orange-400 font-bold text-xs md:text-sm">점심 시간 <span className="text-[10px] font-normal opacity-80 ml-1">({settings.lunchStart} ~ {settings.lunchEnd})</span></div>
                        </td>
                      </tr>
                    )}
                  </React.Fragment>
        ...
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-2xl w-full max-w-sm animate-in zoom-in-95">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-bold text-lg dark:text-white flex items-center gap-2"><span className="bg-indigo-100 text-indigo-700 px-2 py-1 rounded-lg text-sm">{modalData.day}요일 {modalData.period}교시</span></h3>
            ...
            <div className="space-y-5">
              <div>
                ...
                {timetable?.recentSubjects?.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {timetable.recentSubjects.map(sub => <button key={sub} onClick={() => setModalData({...modalData, subject: sub})} className="px-2.5 py-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-[10px] font-bold rounded-full hover:bg-indigo-100 hover:text-indigo-600 transition border dark:border-gray-600">{sub}</button>)}
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                {timetable?.recentRooms?.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {timetable.recentRooms.map(rm => <button key={rm} onClick={() => setModalData({...modalData, room: rm})} className="px-2.5 py-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-[10px] font-bold rounded-full hover:bg-indigo-100 hover:text-indigo-600 transition border dark:border-gray-600">{rm}</button>)}
                  </div>
                )}
              </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-2xl w-full max-w-md animate-in zoom-in-95">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold text-lg dark:text-white flex items-center gap-2"><FileSpreadsheet className="text-green-500"/> 시간표 XLSX 연동</h3>
              <button onClick={() => setXlsxModalOpen(false)} className="text-gray-400 hover:bg-gray-100 rounded-full p-1"><X size={20}/></button>
            </div>
            
            <div className="text-sm text-gray-700 dark:text-gray-300 bg-green-50 dark:bg-green-900/20 p-5 rounded-xl mb-6 border border-green-100 dark:border-green-800/50 leading-relaxed">
              <p className="font-bold mb-3 text-green-700 dark:text-green-400">💡 NEIS 시간표 연동 가이드</p>
              <ol className="list-decimal pl-5 space-y-1.5 font-medium">
                <li><span className="bg-white dark:bg-gray-700 px-1 rounded">나이스 - 교과담임 - 시간표관리 - 교사별시간표조회</span> 진입</li>
                <li><span className="bg-white dark:bg-gray-700 px-1 rounded">교사별출력 - 성함작성 - 조회</span> 클릭</li>
                <li>상단의 <span className="text-green-600 dark:text-green-400 font-bold border-b border-green-400">저장(xlsx)</span> 버튼을 눌러 엑셀 다운로드</li>
                <li>아래 <b>업로드</b> 버튼을 눌러 파일 선택!</li>
              </ol>
            </div>

            <div className="flex flex-col gap-3">
              <button onClick={() => xlsxInputRef.current.click()} className="w-full bg-green-600 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 hover:bg-green-700 transition shadow-md">
                <Upload size={20}/> 엑셀 파일(XLSX) 업로드
              </button>
              <button onClick={handleDownloadTimetableTemplate} className="w-full bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 hover:bg-gray-200 dark:hover:bg-gray-600 transition">
