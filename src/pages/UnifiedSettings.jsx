...
import { Settings, Book, HelpCircle, Database, Info, Grid, Search, GraduationCap, Sun, Moon, Monitor, Sparkles, ExternalLink, Save, Trash2, ChevronDown } from 'lucide-react';
...
export default function UnifiedSettings({ store, handbook, onUpdateHandbook, onDeleteHandbook }) {
    ...
    if (handbook) {
      setFormData({
        ...
        schoolInfo: {
            ...
            officeCode: handbook.schoolInfo?.officeCode || '', grade: String(handbook.schoolInfo?.grade || '1'), class: String(handbook.schoolInfo?.class || '1')
        }
      });
  ...
  const searchSchool = async () => {
    ...
    } catch (e) { showToast("오류가 발생했습니다.", 'error'); } finally { setIsSearching(false); }
  };
  ...
  const handleFontSizeChange = (val) => {
    ...
    if (!isNaN(size) && size >= 10 && size <= 30) store.setFontSize(size);
  };
  const handleFontSizeBlur = () => {
    let size = parseInt(localFontSize);
    if (isNaN(size) || size < 10) size = 10; if (size > 30) size = 30;
    setLocalFontSize(size); store.setFontSize(size);
  };
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-gray-900 overflow-hidden animate-in fade-in duration-300 relative">
            ...
            <div className="max-w-3xl mx-auto space-y-8 pb-10">
              <form onSubmit={handleHandbookSave} className="space-y-6">
                 ...
                 <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 space-y-6">
                   ...
                   <div className="space-y-3">
                     ...
                     <div className="grid grid-cols-3 gap-3">
                       <button type="button" onClick={() => store.setTheme('light')} className={`py-3 rounded-xl flex flex-col items-center gap-1 transition border-2 ${store.theme === 'light' ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-gray-200 dark:border-gray-700 text-gray-500 hover:bg-gray-50'}`}><Sun size={20}/> <span className="text-sm font-bold">라이트</span></button>
                       <button type="button" onClick={() => store.setTheme('dark')} className={`py-3 rounded-xl flex flex-col items-center gap-1 transition border-2 ${store.theme === 'dark' ? 'border-indigo-600 bg-indigo-50 text-indigo-700 dark:text-indigo-400 dark:bg-indigo-900/30' : 'border-gray-200 dark:border-gray-700 text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-700'}`}><Moon size={20}/> <span className="text-sm font-bold">다크</span></button>
                       <button type="button" onClick={() => store.setTheme('system')} className={`py-3 rounded-xl flex flex-col items-center gap-1 transition border-2 ${store.theme === 'system' ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-gray-200 dark:border-gray-700 text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-700'}`}><Monitor size={20}/> <span className="text-sm font-bold">시스템</span></button>
                     </div>
                   </div>
                   <div className="space-y-3">
                     ...
                     <div className="flex items-center gap-4 bg-gray-50 dark:bg-gray-700/50 p-4 rounded-xl border border-gray-200 dark:border-gray-600">
                       ...
                       <div className="flex items-center gap-1 shrink-0">
                         <input type="number" min="10" max="30" value={localFontSize} onChange={e => handleFontSizeChange(e.target.value)} onBlur={handleFontSizeBlur} className="w-16 p-2 text-lg font-black border rounded-lg dark:bg-gray-800 dark:text-white text-center focus:ring-2 focus:ring-indigo-500 outline-none transition" />
                         <span className="text-gray-500 font-bold text-sm">pt</span>
                       </div>
                     </div>
                   </div>
                   <div className="space-y-3">
                     <div className="flex justify-between items-center"><label className="text-sm font-bold text-gray-700 dark:text-gray-300 flex items-center gap-1"><Sparkles className="text-yellow-500" size={16}/> Gemini API Key (선택)</label><a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noreferrer" className="text-xs text-indigo-600 hover:underline font-bold flex items-center gap-1">API 키 발급 <ExternalLink size={12}/></a></div>
                     <input type="password" value={store.apiKey} onChange={(e) => store.setApiKey(e.target.value)} placeholder="AI 기능을 사용하려면 API 키를 입력하세요" className="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-yellow-500 outline-none" />
                   </div>
                 </div>
                 ...
                 <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 space-y-6">
                   ...
                   <div className="space-y-2"><label className="block text-sm font-bold text-gray-700 dark:text-gray-300">교무수첩 이름</label><input type="text" required value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} className="w-full p-3 border rounded-xl dark:bg-gray-700 dark:text-white outline-none" /></div>
                   <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-xl flex items-center justify-between border dark:border-gray-600">
                     <div className="flex items-center gap-3"><div className="w-10 h-10 bg-indigo-100 dark:bg-indigo-900/50 rounded-full flex items-center justify-center text-indigo-600"><GraduationCap size={20}/></div><div><p className="font-bold text-sm dark:text-white">담임 선생님이신가요?</p><p className="text-xs text-gray-500">우리반 관리 기능을 활성화합니다.</p></div></div>
                   ...
                   <div className="space-y-4">
                     <div className="relative">
                       ...
                       <div className="flex gap-2"><input type="text" value={schoolSearchName} onChange={(e) => setSchoolSearchName(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); searchSchool(); } }} placeholder="학교명 검색 (예: 서울초)" className="flex-1 p-3 border rounded-xl dark:bg-gray-700 dark:text-white outline-none" /><button type="button" onClick={searchSchool} disabled={isSearching} className="bg-indigo-600 text-white p-3 rounded-xl hover:bg-indigo-700 disabled:opacity-50"><Search size={20}/></button></div>
                       {searchResults.length > 0 && (<div className="mt-2 max-h-40 overflow-y-auto border rounded-xl bg-white dark:bg-gray-700 absolute z-50 w-full shadow-xl">{searchResults.map((s, idx) => (<div key={idx} onClick={() => handleSelectSchool(s)} className="p-3 hover:bg-indigo-50 dark:hover:bg-gray-600 cursor-pointer text-sm border-b dark:border-gray-600 transition"><p className="font-bold dark:text-white">{s.name}</p><p className="text-xs text-gray-500">{s.address}</p></div>))}</div>)}
                     ...
                     {formData.isHomeroom && (
                       <div className="grid grid-cols-2 gap-4">
                         <div><label className="block text-sm font-bold mb-1 dark:text-white">학년</label><select value={String(formData.schoolInfo.grade)} onChange={(e) => setFormData({...formData, schoolInfo: {...formData.schoolInfo, grade: e.target.value}})} className="w-full p-3 border rounded-xl outline-none dark:bg-gray-700 dark:text-white">{[1,2,3,4,5,6].map(g=><option key={g} value={String(g)}>{g}학년</option>)}</select></div>
                         <div><label className="block text-sm font-bold mb-1 dark:text-white">반</label><select value={String(formData.schoolInfo.class)} onChange={(e) => setFormData({...formData, schoolInfo: {...formData.schoolInfo, class: e.target.value}})} className="w-full p-3 border rounded-xl outline-none dark:bg-gray-700 dark:text-white">{Array.from({length: 20}, (_, i) => i + 1).map(c=><option key={c} value={String(c)}>{c}반</option>)}</select></div>
                       </div>
                     )}
                   </div>
                 </div>
              ...
              {handbook && (<div className="pt-4 shrink-0"><button type="button" onClick={handleDelete} className="w-full flex justify-center gap-2 p-4 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl font-bold"><Trash2 size={18}/> 이 교무수첩 삭제하기</button></div>)}
            </div>
         )}
...
function QnASection() {
  const faqs = [
    ...
    { q: "AI 세특 자동 작성은 어떻게 쓰나요?", a: "통합 설정 > 환경 설정에서 발급받은 'Gemini API Key'를 입력하신 후, 학생 명렬표에서 학생의 '태그'나 '특기사항'을 입력하고 우측 하단의 [AI 세특] 버튼을 누르시면 3문장으로 깔끔하게 요약해 줍니다." },
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4">
      <h3 className="font-bold text-2xl mb-6 dark:text-white">💡 무엇을 도와드릴까요?</h3>
      {faqs.map((faq, idx) => (
        <details key={idx} className="group bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden cursor-pointer">
          <summary className="font-bold p-5 flex justify-between items-center text-gray-800 dark:text-white outline-none">
            <span className="flex items-center gap-2"><span className="text-indigo-500">Q.</span> {faq.q}</span>
