...
import { Settings, Book, HelpCircle, Database, Info, Grid, X, Search, GraduationCap, Sun, Moon, Monitor, Sparkles, ExternalLink, Save, Trash2, ChevronDown, UserCircle, LogOut } from 'lucide-react';
...
export default function UnifiedSettings({ isOpen, onClose, store, handbook, onUpdateHandbook, onDeleteHandbook, user, logout }) {
  ...
  const tabs = [
    ...
    { id: 'account', label: '계정 및 연동 관리', icon: UserCircle }, // 🔥 신규 탭 추가
                   ...
                   <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 space-y-6">
                     ...
                     <div className="space-y-3">
                       ...
                       <div className="grid grid-cols-3 gap-3">
                         ...
                         <button type="button" onClick={() => store.setTheme('dark')} className={`py-3 rounded-xl flex flex-col items-center gap-1 transition border-2 ${store.theme === 'dark' ? 'border-indigo-600 bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400' : 'border-gray-200 dark:border-gray-700 text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-700'}`}><Moon size={20} className={store.theme === 'dark' ? 'text-indigo-600 dark:text-indigo-400' : ''}/> <span className="text-sm font-bold">다크</span></button>
                   ...
                   <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 space-y-6">
                     ...
                     <div className="space-y-4">
                       <div className="relative">
                         ...
                         {searchResults.length > 0 && (<div className="mt-2 max-h-40 overflow-y-auto border border-gray-200 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-700 absolute z-50 w-full shadow-xl custom-scrollbar">{searchResults.map((s, idx) => (<div key={idx} onClick={() => handleSelectSchool(s)} className="p-3 hover:bg-indigo-50 dark:hover:bg-gray-600 cursor-pointer text-sm border-b border-gray-100 dark:border-gray-600 transition"><p className="font-bold dark:text-white">{s.name}</p><p className="text-xs text-gray-500 dark:text-gray-400">{s.address}</p></div>))}</div>)}
           ...
           {activeTab === 'account' && (
             <div className="max-w-2xl mx-auto space-y-8 pb-10">
               ...
               <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 text-center">
                 ...
                 <button onClick={logout} className="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-6 py-2.5 rounded-xl font-bold hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/30 dark:hover:text-red-400 transition flex items-center justify-center gap-2 mx-auto">
               ...
               <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 space-y-4">
                 <div className="flex justify-between items-center border-b border-gray-100 dark:border-gray-700 pb-3">
                   <h3 className="text-sm font-bold text-gray-700 dark:text-gray-300 flex items-center gap-2"><Database className="text-orange-500" size={18}/> Firebase 설정 (선택사항)</h3>
                 </div>
                 <textarea 
                   value={store.firebaseConfig || ''} 
                   onChange={(e) => store.setFirebaseConfig(e.target.value)} 
                   placeholder="사용자 지정 Firebase Config JSON을 입력하세요 (비워두면 기본값 사용)" 
                   className="w-full h-32 p-3 border border-gray-300 rounded-xl dark:bg-gray-700 dark:border-gray-600 dark:text-white font-mono text-xs focus:ring-2 focus:ring-orange-500 outline-none resize-none custom-scrollbar" 
                 />
                 <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">
                   독립적인 실시간 데이터베이스를 구축하고 싶으신 분만 본인의 Firebase Config 객체를 JSON 형태로 입력하세요.<br/>
                   (입력 후 앱을 새로고침해야 적용됩니다.)
                 </p>
               </div>
