...
export default function MemoPage() {
  ...
  const handleAdd = () => {
    ...
    addMemo({ content: newMemoText, color: randomColor });
    setNewMemoText("");
  };
          ...
          {memos.map(memo => (
            <div key={memo.id} className={`${memo.color} dark:text-gray-900 p-4 rounded-md shadow-[2px_4px_6px_rgba(0,0,0,0.1)] relative group aspect-square flex flex-col transform hover:scale-105 transition-transform rotate-1 hover:rotate-0 hover:z-10`}>
              <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition">
                <button onClick={() => deleteMemo(memo.id)} className="text-gray-500 hover:text-red-500 p-1 bg-white/50 rounded-full"><X size={14}/></button>
              </div>
              <textarea 
                value={memo.content} 
              ...
              />
              <div className="text-[10px] text-gray-500 text-right mt-2">{new Date(memo.id).toLocaleDateString()}</div>
            </div>
          ))}
